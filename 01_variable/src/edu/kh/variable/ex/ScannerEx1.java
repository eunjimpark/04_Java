...
public class ScannerEx1 {
	public static void main(String[] args) {
		 ...
		 * ->import구문 작성 필요
		 * 
		 * -Scanner.next() : 단어 1개 입력, 공백문자입력시 입력 종료
		 * 
		 * -Scanner.nextLine() : 문자열 1개 입력 엔터(개행 문자)입력시 종료
		 * 
		...
		//Scanner 사용하기
		//1. 클래스 위해 import 하는 구문 작성
		//2. import한 Scanner클래스를 이용해서 Scanner 객체 생성
		...
		// 
		// nextLine()을 작성해야하는 상황이 오면 nextLine()구문을 미리 한번 작성하기
		
		sc.nextLine();
		
		//Scanner사용시 입력되는 값이 Scanner의 입력 버퍼 라는 곳에 젖아된 후 
		//nextInt()면 버퍼에 저장된 값 중 정수만 추출 
		// ->이때 정수와 같이 입력된 엔터가 버퍼에 남아있게됨
		//이후 nextLine()해석 시 버퍼에서 다음 엔터까지 읽어오는 작업이 수행되는데
